...
TIGR_API void tigrSetPostFX(Tigr *bmp, float p1, float p2, float p3, float p4);
...
TIGR_API TPixel tigrGet(Tigr *bmp, int x, int y);

// Plots a pixel.
// Clips and blends.
// For high performance, just access bmp->pix directly.
TIGR_API void tigrPlot(Tigr *bmp, int x, int y, TPixel pix);

// Clears a bitmap to a color.
// No blending, no clipping.
TIGR_API void tigrClear(Tigr *bmp, TPixel color);

// Fills a rectangular area.
// No blending, no clipping.
TIGR_API void tigrFill(Tigr *bmp, int x, int y, int w, int h, TPixel color);

// Draws a line.
// Start pixel is drawn, end pixel is not.
// Clips and blends.
TIGR_API void tigrLine(Tigr *bmp, int x0, int y0, int x1, int y1, TPixel color);

// Draws an empty rectangle.
// Drawing a 1x1 rectangle yields the same result as calling tigrPlot.
// Clips and blends.
TIGR_API void tigrRect(Tigr *bmp, int x, int y, int w, int h, TPixel color);
...
// Clips and blends.
TIGR_API void tigrFillRect(Tigr *bmp, int x, int y, int w, int h, TPixel color);
...
// Clips and blends.
TIGR_API void tigrCircle(Tigr *bmp, int x, int y, int r, TPixel color);
...
// Clips and blends.
TIGR_API void tigrFillCircle(Tigr *bmp, int x, int y, int r, TPixel color);

// Sets clip rect.
// Set to (0, 0, -1, -1) to reset clipping to full bitmap.
TIGR_API void tigrClip(Tigr *bmp, int cx, int cy, int cw, int ch);
...
TIGR_API void tigrBlit(Tigr *dest, Tigr *src, int dx, int dy, int sx, int sy, int w, int h);
...
// Clips and blends.
TIGR_API void tigrBlitAlpha(Tigr *dest, Tigr *src, int dx, int dy, int sx, int sy, int w, int h, float alpha);
...
// Clips and blends.
TIGR_API void tigrBlitTint(Tigr *dest, Tigr *src, int dx, int dy, int sx, int sy, int w, int h, TPixel tint);
...
TIGR_API void tigrBlitMode(Tigr *dest, int mode);
...
//
TIGR_API TigrFont *tigrLoadFont(Tigr *bitmap, int codepage);

// Frees a font.
TIGR_API void tigrFreeFont(TigrFont *font);

// Prints UTF-8 text onto a bitmap.
// NOTE:
//  This uses the target bitmap blit mode.
//  See tigrBlitTint for details.
TIGR_API void tigrPrint(Tigr *dest, TigrFont *font, int x, int y, TPixel color, const char *text, ...);

// Returns the width/height of a string.
TIGR_API int tigrTextWidth(TigrFont *font, const char *text);
TIGR_API int tigrTextHeight(TigrFont *font, const char *text);

// The built-in font.
TIGR_API extern TigrFont *tfont;
...
TIGR_API void tigrMouse(Tigr *bmp, int *x, int *y, int *buttons);
...
// silly to expose this but for completeness
TIGR_API int tigrTouch(Tigr *bmp, TigrTouchPoint* points, int maxPoints);
...
TIGR_API int tigrKeyDown(Tigr *bmp, int key);
TIGR_API int tigrKeyHeld(Tigr *bmp, int key);

// Reads character input for a window.
// Returns the Unicode value of the last key pressed, or 0 if none.
TIGR_API int tigrReadChar(Tigr *bmp);

// Show / hide virtual keyboard.
// (Only available on iOS / Android)
TIGR_API void tigrShowKeyboard(int show);
...
TIGR_API Tigr *tigrLoadImage(const char *fileName);
TIGR_API Tigr *tigrLoadImageMem(const void *data, int length);

// Saves a PNG to a file. (fileName is UTF-8)
// On error, returns zero and sets errno.
TIGR_API int tigrSaveImage(const char *fileName, Tigr *bmp);
...
TIGR_API float tigrTime(void);

// Displays an error message and quits. (UTF-8)
// 'bmp' can be NULL.
TIGR_API void tigrError(Tigr *bmp, const char *message, ...);
...
TIGR_API void *tigrReadFile(const char *fileName, int *length);

// Decompresses DEFLATEd zip/zlib data into a buffer.
// Returns non-zero on success.
TIGR_API int tigrInflate(void *out, unsigned outlen, const void *in, unsigned inlen);

// Decodes a single UTF8 codepoint and returns the next pointer.
TIGR_API const char *tigrDecodeUTF8(const char *text, int *cp);

// Encodes a single UTF8 codepoint and returns the next pointer.
TIGR_API char *tigrEncodeUTF8(char *text, int cp);
